...
class _FilterBuilder:
    ...
    def __init__(self, field_names: list[str]) -> None:
        ...
        with ui.row().classes("gap-2 items-end"):
            self._field_input = ui.select(
                field_names, label="Field", with_input=True,
            ).classes("w-48")
    ...
    def _add_clause(self) -> None:
        ...
        clause_str = (
            f'[ViewFilter({clause_num}, F, , '
            f'"{field}", "{qual}", "{value}", False)]'
        )
        self._clauses.append({
            "field": field, "qual": qual, "value": value, "str": clause_str,
        })
        ...
        with self._clauses_display:
            for i, c in enumerate(self._clauses):
                with ui.row().classes("items-center gap-2"):
                    ui.label(
                        f"{i + 1}. {c['field']} {c['qual']} {c['value']!r}"
                    ).classes("text-sm")
                    ui.button(
                        icon="close",
