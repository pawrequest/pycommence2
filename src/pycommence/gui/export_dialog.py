        ...
        with ui.column().classes('gap-4 w-full'):
            ...
            async def on_category_change(e) -> None:
                ...
                if cat not in field_names_cache:
                    try:
                        names = await worker.run(lambda s, c=cat: s.schema.get_field_names(c))
    ...
    async def do_export() -> None:
        ...
        try:
            count = await worker.run(
                lambda s, c=cat, p=path, f=fmt, cl=cols, li=limit, cn=canonical: s.export(
                    c, p, format=f, columns=cl, max_rows=li, canonical=cn
                )
            )
