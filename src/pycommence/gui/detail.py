...
async def _render_detail(category: str, row_id: str) -> None:
    ...
    # Load the row data
    try:
        row = await worker.run(
            lambda s, c=category, rid=row_id: s.read_by_id(c, rid)
        )
    ...
    async def save_changes() -> None:
        ...
        try:
            await worker.run(
                lambda s, c=category, rid=row_id, ch=changes: s.edit(rid, c, ch)
            )
    ...
    with ui.row().classes("gap-4 mt-4"):
        ...
        async def open_in_commence() -> None:
            pk_field = next(
                (f.name for f in fields if f.field_type.name == "NAME"), None
            )
            pk_value = row.columns.get(pk_field, "") if pk_field else ""
            if pk_value:
                try:
                    await worker.run(
                        lambda s, c=category, pk=pk_value: s.dde.show_item(c, pk)
                    )
    ...
    if connections:
        ...
        if pk_value:
            for conn in connections:
                with ui.expansion(
                    f"{conn.name} → {conn.to_category}", icon="link"
                ).classes("w-full"):
                    try:
            ...
            ui.label(
                "Cannot load connections — item name field not found."
