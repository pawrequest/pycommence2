...
Cache files are stored under ``platformdirs.user_cache_dir("pycommence")``,
...
class SchemaCache:
    ...
    def check_staleness(self, live_category_count: int | None = None) -> str:
        ...
        # L0: DB identity
        if (
            self._data.get("db_name") != self.db_name
            or self._data.get("db_path") != self.db_path
        ):
        ...
        if cached_mtime and current_mtime:
            if cached_mtime == current_mtime:
                ...
                log.info(
                    "Cache L1 fail: METADATA.PIM mtime changed "
                    "(cached=%s, current=%s)",
        ...
        if live_category_count is not None:
            ...
            if cached_count is not None and cached_count == live_category_count:
                ...
                log.info(
                    "Cache L2 fail: category count changed "
                    "(cached=%s, live=%s)",
                    cached_count, live_category_count,
                )
                self.trust_level = TRUST_NONE
                return TRUST_NONE
