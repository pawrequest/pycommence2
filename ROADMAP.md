...
**Docs before GUI? Yes.**  A GUI built on an undocumented, shifting API creates a maintenance nightmare. Writing docs
first forces us to name things well, spot gaps, and stabilise. The GUI then writes itself against a clean surface.
...
Add sections for: view cursors (`query_view`), related columns, canonical mode, DDE, connections. The README is
...
Expose `read`, `query`, `schema`, `add`, `edit`, `delete`, `connections` as MCP tools, allowing LLM agents to interact
with Commence databases. New module `src/pycommence/mcp_server.py`.  (There's already an external MCP server — this
would be built-in.)

### 6.2  Async wrapper ✅

COM is STA-bound. Create `AsyncCommenceSession` that dispatches all calls to a dedicated thread via
`asyncio.to_thread()`:
