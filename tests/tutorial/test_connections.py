...
class TestConnectionQuery:
    ...
    def test_get_connected_item_count(self, session: CommenceSession) -> None:
        ...
        count = session.connections.get_connected_item_count(
            CONTACT_CAT, "Gates.Bill", CONNECTION_NAME, ACCOUNT_CAT,
        )
...
class TestConnectionAssignUnassign:
    ...
    def test_assign_and_unassign(self, session: CommenceSession) -> None:
        ...
        try:
            ...
            session.assign_connection(
                CONTACT_CAT, contact_name, CONNECTION_NAME,
                ACCOUNT_CAT, account_name,
            )

            # Verify
            names = session.connections.get_connected_item_names(
                CONTACT_CAT, contact_name, CONNECTION_NAME, ACCOUNT_CAT,
            )
    ...
    @staticmethod
    def _delete_by_name(
        session: CommenceSession, category: str, key_field: str, name: str,
    ) -> None:
        rows = (
            session.query(category)
            .columns(key_field)
            .where(key_field, "Equal To", name)
            .execute()
        )
