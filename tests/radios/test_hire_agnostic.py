...
class TestSchemaDiscovery:
    ...
    def test_field_count_matches_names(
        self, session: CommenceSession, field_names: list[str],
    ) -> None:
...
class TestSketchyNames:
    ...
    def test_sketchy_names_exist(self, sketchy_names: list[str]) -> None:
        ...
        log.info("Found %d sketchy names (showing first 5): %s",
                 len(sketchy_names), sketchy_names[:5])
    ...
    def test_query_filter_contains_on_sketchy_rows(
        ...
        assert target in matched_names, (
            f"Expected {target!r} in results for Contains {substring!r}"
        )
...
class TestDdeCursorConsistency:
    ...
    def test_item_count_matches_row_count(
        self, session: CommenceSession, total_rows: int,
    ) -> None:
...
class TestQueryBuilderAgnostic:
    ...
    def test_sort_by_pk(
        self, session: CommenceSession, pk_field: str,
    ) -> None:
    ...
    def test_filter_reduces_count(
        self, session: CommenceSession, pk_field: str, total_rows: int,
    ) -> None:
        """A restrictive filter must return fewer rows than the total."""
        count = (
            session.query(CATEGORY)
            .where(pk_field, "Equal To", "ZZZNONEXISTENT999")
            .count()
        )
        assert count < total_rows
    ...
    def test_query_with_date_field_filter(
        self, session: CommenceSession, schema_fields: list[FieldInfo], pk_field: str,
    ) -> None:
