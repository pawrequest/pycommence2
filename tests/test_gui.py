...
class TestComWorkerStartStop:
    ...
    def _make_worker(self):
        ...
        from pycommence.gui.state import ComWorker
        return ComWorker()
    ...
    def test_start_sets_connected(self) -> None:
        ...
        with patch("pycommence.session.CommenceSession.__new__", return_value=mock_session):
            with patch("pycommence.session.CommenceSession.__init__", return_value=None):
                worker.start()
        try:
    ...
    def test_start_records_error_on_failure(self) -> None:
        worker = self._make_worker()
        with patch(
            "pycommence.session.CommenceSession.__init__",
...
class TestAppState:
    ...
    def test_default_edit_unlocked(self) -> None:
        from pycommence.gui.state import AppState
        state = AppState()
...
class TestFilterBuilderLogic:
    ...
    def _make_builder(self, field_names=None):
        ...
        with patch("pycommence.gui.browser.ui") as mock_ui:
            ...
            from pycommence.gui.browser import _FilterBuilder
    ...
    def test_add_clause_builds_filter_string(self) -> None:
        ...
        fb._clauses.append({
            "field": "Name", "qual": "Contains", "value": "Smith",
    ...
    def test_multiple_clauses(self) -> None:
        ...
        for i, (field, qual, val) in enumerate([
            ...
            fb._clauses.append({
                "field": field, "qual": qual, "value": val,
    ...
    def test_max_four_clauses(self) -> None:
        ...
        for i in range(4):
            fb._clauses.append({
                "field": "Name", "qual": "Contains", "value": f"val{i}",
...
class TestCreateApp:
    ...
    def test_create_app_starts_worker(self) -> None:
        ...
        with (
            patch("pycommence.gui.state.ComWorker", mock_worker_cls),
            patch("pycommence.gui.browser.register"),
            patch("pycommence.gui.dashboard.register"),
            patch("pycommence.gui.detail.register"),
            patch("pycommence.gui.schema_explorer.register"),
            patch("pycommence.gui.export_dialog.register"),
            patch("pycommence.gui.dde_console.register"),
            patch("pycommence.gui.app", create=True),
        ):
            # Need to also patch the nicegui import
            with patch.dict("sys.modules", {"nicegui": MagicMock(), "nicegui.app": MagicMock()}):
                import pycommence.gui as gui_mod
...
class TestLayoutHelpers:
    ...
    def test_notify_error(self) -> None:
        with patch("pycommence.gui.layout.ui") as mock_ui:
            from pycommence.gui.layout import notify_error
    ...
    def test_notify_success(self) -> None:
        with patch("pycommence.gui.layout.ui") as mock_ui:
            from pycommence.gui.layout import notify_success
    ...
    def test_set_edit_mode(self) -> None:
        from pycommence.gui.layout import _set_edit_mode
        from pycommence.gui.state import app_state
...
class TestDdeConsoleTemplates:
    ...
    def test_templates_are_bracketed_commands(self) -> None:
        ...
        with patch("pycommence.gui.dde_console.ui"):
            with patch("pycommence.gui.dde_console.frame"):
                with patch("pycommence.gui.dde_console.worker"):
                    # Module loaded — that's enough for a smoke test
                    from pycommence.gui import dde_console
